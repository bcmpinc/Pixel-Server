...
bool writeimage(agg::rendering_buffer &rbuf, std::stringstream &s) {
	bool ret = false;
	png_bytep row_pointers[rbuf.height()];
	...
	if (png_ptr && info_ptr && !setjmp(png_jmpbuf(png_ptr))) {
		...
		png_set_IHDR(png_ptr, info_ptr, rbuf.width(), rbuf.height(), 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
		for (unsigned int i = 0; i < rbuf.height(); i++) row_pointers[i] = rbuf.row_ptr(i);
